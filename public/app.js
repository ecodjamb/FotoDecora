...
function updateAuth(){const b=$('#authBtn');if(b)b.textContent=state.session?.user?.email?'Mi cuenta':'Ingresar'}
...
async function optimizeImage(file){
  ...
  if(file.size>25e6)throw new Error('La foto original supera 25 MB. Usa una foto más liviana.');
  const data=await new Promise((ok,no)=>{const r=new FileReader();r.onload=()=>ok(r.result);r.onerror=no;r.readAsDataURL(file)});
  const img=await new Promise((ok,no)=>{const x=new Image();x.onload=()=>ok(x);x.onerror=no;x.src=data});
...
async function selectPhoto(file){
  if(!file)return;
  try{toast('Preparando foto…');state.photo=await optimizeImage(file);$('#photoPreview').src=state.photo;section('wizard')}
  catch(e){toast(e.message||'No pudimos abrir la foto.')}
}
...
function animateLoading(){
  let p=10,i=0;const bar=$('#progressBar');
  const texts=['Analizando luz, distribución y estilo…','Buscando productos reales en Chile…','Armando tres niveles de presupuesto…'];
  const details=['Leyendo proporciones y elementos de la foto.','Revisando tiendas y piezas compatibles.','Calculando cantidades y presupuesto total.'];
  if(bar)bar.style.width=p+'%';
  const t=setInterval(()=>{if($('#loading')?.classList.contains('hidden'))return clearInterval(t);p=Math.min(92,p+Math.random()*10);if(bar)bar.style.width=p+'%';i=Math.min(i+1,texts.length-1);$('#loadingTitle').textContent=texts[i];$('#loadingDetail').textContent=details[i]},3500)
}
...
async function generateImage(i){
  ...
  try{
    const r=await fetch('/api/decorate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({image:state.photo,prompt:t.image_prompt,tier:t.level,room:state.brief?.room,keep:state.brief?.keep})});
    const d=await r.json();
    if(!r.ok)throw new Error(d.error||'No se pudo generar la imagen');
    state.images[i]=d.image;
  }catch(e){state.images[i]={error:e.message||'No se pudo generar la imagen'}}
  finally{state.generating.delete(i);renderTierNav();if(state.tier===i)renderTier()}
}

function setTier(i){
  const n=Number(i);if(!Number.isInteger(n)||n<0||n>2||!state.plan?.tiers?.[n])return;
  state.tier=n;renderTierNav();renderTier();window.scrollTo({top:Math.max(0,($('#tabs')?.getBoundingClientRect().top||0)+window.scrollY-90),behavior:'smooth'});
}
...
async function ensureSession(force=true){
  if(!state.session)return false;const exp=(state.session.expires_at||0)*1000;
  if(!force&&(!exp||exp>Date.now()+60000))return true;if(exp>Date.now()+60000)return true;if(!state.session.refresh_token)return false;
